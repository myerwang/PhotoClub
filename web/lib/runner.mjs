...
export function buildGeneratePrompt({ rootDir = process.cwd(), profileIds, styleId, format, orientation = 'portrait', extraPrompt = '', quantity = 1, outputPaths }) {
  ...
  return `在照片项目中生成最终风格照片。\n\n${imageGenerationRequirements()}\n\n身份输入：只使用以下人物多视图文件，禁止读取或使用 input 目录原图：\n${identities}\n\n每个人只能对应自己的多视图参考。直接把所有列出的多视图参考图一并喂给图像模型；不要用文字描写或重建五官；不得混合、互换、复制或遗漏人物身份。每张结果都必须让所有选中人物同时出现在同一张照片中，形成多人物合照。\n身份一致性硬要求：最终照片中的脸必须是所选多视图参考里的同一个人，不能只是相似、年轻化、老化、美化、网红化、名人化、通用模特脸或同族裔替代脸。使用多视图参考作为脸部身份锚点；风格只能改变服装、场景、镜头、灯光、姿态、材质和后期质感。如果风格效果会导致脸不像本人，必须降低风格强度，优先保住人脸身份。完整脸部必须在画面内，不得裁掉头顶、额头、眼睛、鼻子、嘴、下巴或脸部任一侧。\n服饰硬要求：禁止参照人物设定中的服饰；人物多视图参考里的素色衣服、颜色、领口、材质和证件照式穿着都不是身份的一部分，任何条件下都不能复制到最终照片。若 style 明确包含服饰设定，严格使用 style 的服饰语言，不得替换、弱化、重解释或用系统默认服饰干扰；若 style 没有包含服饰设定，自动选择最适合该照片风格的服饰，需匹配场景、年代、天气、镜头、灯光和编辑氛围。\n风格规则：读取并严格执行 ${stylePath}。风格规则不得限制人物属性。style_id、文件名和文件路径只是内部标识，不是生成语义；不得从标识里的 woman、girl、man、group、portrait、celebrity、race、place 等词推断人物性别、人数、身份、族裔或固定对象。\n风格文件中的 ## Source Prompt 只作为来源记录，不是生产提示词；不要执行其中的固定人物、年龄、性别、族裔、五官、主体数量、名人、遮脸、模糊、马赛克、黑条、手机挡脸、面具、头盔、变成异种或其他会覆盖身份参考的内容。只提取摄影、构图、镜头、灯光、色彩、场景、服装语言、材质和后期处理。最终照片必须保留足够清晰可见的人脸用于核对身份。\n输出格式：${format.id}，照片方向为${orientationLabel}，每张最终文件必须原生构图并精确生成为 ${format.width} x ${format.height} 像素。不得先按另一方向生成后旋转或裁切。不要因照片方向增加白边或通用安全边距；安全边距仅在所选风格规则明确要求时应用。\n本次临时要求：${extraPrompt.trim() || '无'}。本次要求只用于当前任务，不得写回风格文件。\n生成数量：严格生成 ${quantity} 张。每张图片必须分别调用一次内置 image_gen，不能把多张结果拼成一张图。\n最终路径按顺序保存：\n${targets}\n验证每个目标文件均存在且可读后才能结束任务。`;
}
