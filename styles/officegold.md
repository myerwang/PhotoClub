---
style_id: officegold
name: 黎明办公室金光
source_url: https://note.com/old_pgmrs_will/n/n1b2a6b98aa96
source_title: GPT-Image-2最強プロンプト術｜いにしえ@AIクリエイター
source_result: 31
retrieved_at: 2026-06-19T10:59:02.933Z
---

# Style: Dawn Office Gold

## Source Prompt

A Japanese businessman (1) in a dark navy three-piece suit, sharp jawline, confident eyes, mid-30s (2), standing with arms crossed slightly off-center to the right, facing camera (3), modern glass-walled office at dawn with Tokyo skyline visible through windows (4), soft golden hour side lighting from upper left, warm color temperature (5), 85mm f/1.4 lens, shallow depth of field with creamy bokeh, three-quarter view, bust shot (6), photorealistic editorial portrait, cinematic color grading (7). Format 16:9.

## Adaptation Log

- 删除固定国籍、性别、年龄、职业及五官文字描述，人物身份由所选人物参考决定。
- 删除固定纵横比；保留深色正式服装、黎明玻璃办公室、东京天际线、左上金色侧光、85mm 浅景深和电影调色。

## Visual Rules

- 采用来源七要素示例明确给出的写实编辑人像、现代玻璃办公室、正式服装与电影调色。

## Composition

- 人物略偏画面右侧、面向镜头，采用三分之四视角胸像与 85mm f/1.4 浅景深；人物属性和人数由当前任务决定。

## Lighting And Color

- 使用左上方柔和金色时刻侧光、暖色温，窗外保留黎明城市层次和奶油状散景。

## Subject Boundary

- Apply `system/rules/style_base.md`.
- This style defines visual treatment only and does not restrict subject identity, attributes, count, or combinations.
